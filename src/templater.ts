    ...
    function (_, collection, loopVar, internalTemplate) {
      return data[collection] && Array.isArray(data[collection])
        ? (data[collection] as unknown[])
            .map((item: unknown, _index: number, arr: Array<unknown>) => {
              ...
              //console.log(iterationData, arr.length);
              ...
              return template(internalTemplate, updatedItem, iterationData);
            })
            .join("")
        : "";
    },
